...
`define VGA_DBG_Core_Assignments \
    ...
    assign dbg_IfId_pc = PCD; \
    assign dbg_IfId_inst = Ins; \
    assign dbg_IfId_valid = ~StallD; \
    assign dbg_IdEx_pc = PCE; \
    assign dbg_IdEx_inst = InsE; \
    assign dbg_IdEx_valid = ~StallE; \
    assign dbg_IdEx_rd = rdE; \
    assign dbg_IdEx_rs1 = rs1E; \
    assign dbg_IdEx_rs2 = rs2E; \
    assign dbg_IdEx_rs1_val = Op1; \
    assign dbg_IdEx_rs2_val = Op2; \
    assign dbg_IdEx_reg_wen = RegWriteE; \
    assign dbg_IdEx_is_imm = ALUSrcE; \
    assign dbg_IdEx_imm = ImmE; \
    assign dbg_IdEx_mem_wen = MemWriteE; \
    assign dbg_IdEx_mem_ren = MemReadE; \
    assign dbg_IdEx_is_branch = BranchE & ZeroE; \
    assign dbg_IdEx_is_jal  = JumpE[0]; \
    assign dbg_IdEx_is_jalr = JumpE[1]; \
    assign dbg_IdEx_is_auipc = 0; \
    assign dbg_IdEx_is_lui = luiE; \
    assign dbg_IdEx_alu_ctrl = CtrlE; \
    assign dbg_IdEx_cmp_ctrl = 0; \
    assign dbg_ExMa_pc = PCM; \
    assign dbg_ExMa_inst = InsM; \
    assign dbg_ExMa_valid = ~StallM; \
    assign dbg_ExMa_rd = rdM; \
    assign dbg_ExMa_reg_wen = RegWriteM; \
    assign dbg_ExMa_mem_w_data = mem_w_dataM; \
    assign dbg_ExMa_alu_res = ALUOutM; \
    assign dbg_ExMa_mem_wen = MemWriteM; \
    assign dbg_ExMa_mem_ren = 0; \
    assign dbg_ExMa_is_jal = 0; \
    assign dbg_ExMa_is_jalr = 0; \
    assign dbg_MaWb_pc = PCW; \
    assign dbg_MaWb_inst = InsW; \
    assign dbg_MaWb_valid = ~StallW; \
    assign dbg_MaWb_rd = rdW; \
    assign dbg_MaWb_reg_wen = RegWriteW; \
    assign dbg_MaWb_reg_w_data = ResultW;
